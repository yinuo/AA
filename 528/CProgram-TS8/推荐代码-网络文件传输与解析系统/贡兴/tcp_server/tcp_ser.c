...
void *udp_detection (void* arg){
 ...
 //           break;
		...
		if((strcmp(send_buf,recv_buf))>0)
		   send_buf[128]="movielist.txt is renew shoud be download again!";
		else
		   send_buf[128]="movielist.txt is the newest";
          //  printf("UDP_recvfrom(): [%s]\n", recv_buf);
         //   printf("UDP_recvfrom() client IP: [%s]\n", inet_ntoa(cli.sin_addr));
        //    printf("UDP_recvfrom() client PORT: [%d]\n", ntohs(cli.sin_port));
        }
